...
#
#    # 1. CHECK GAMMA FREQ: Analyze the original dyn1 file
#IS_NEGATIVE=$(awk '
#        /q = \( *[0-9.-]+ *[0-9.-]+ *[0-9.-]+ \)/ {
#            if ($4 > -0.0001 && $4 < 0.0001 && $5 > -0.0001 && $5 < 0.0001) { is_gamma=1 }
#            else { is_gamma=0 }
#        }
#        is_gamma && /freq/ && /\=/ && /\-/ { print "YES"; exit }
#    ' "$PREFIX.dyn1")
#
#if [ "$IS_NEGATIVE" == "YES" ]; then
#    echo "⚠️  WARNING: negative frequencies at Gamma in $PREFIX.dyn1"
#    echo "--> Running dynmat.x to generate $PREFIX\_forced.dyn1"
#
#    # Run dynmat.x (ensure your input uses filout = '${PREFIX}_forced.dyn1')
#    $DYN_EXE < ../$INPUT_DIR/dynmat.in > dynmat.out
#
#    # 2. SECOND CHECK: Verify the correction worked
#    STILL_NEGATIVE=$(awk '
#            /q = \( *[0-9.-]+ *[0-9.-]+ *[0-9.-]+ \)/ {
#                if ($4 > -0.0001 && $4 < 0.0001 && $5 > -0.0001 && $5 < 0.0001) { is_gamma=1 }
#                else { is_gamma=0 }
...
#        ' "${PREFIX}_forced.dyn1")
#
#    if [ "$STILL_NEGATIVE" == "YES" ]; then
#        echo "❌ ERROR: Frequencies are STILL negative in ${PREFIX}_forced.dyn1."
#        echo "This is a physical instability. Rerun relax/scf with tighter thresholds."
#    else
#        echo "✅ ASR correction successful. "
#    fi
#else
#    echo "✅ No negative frequencies detected at Gamma. No correction needed."
#fi
#
#cd ..
