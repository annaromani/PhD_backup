...
########## to call inside the directory ph_*
    ...
    head -n 1 $RUN_PATH/nscf/"Q-points_IBZ.txt" >> $PREFIX.dyn0
    
    # Copy q-coords only (columns 1, 2, 3) from the formatted text file
    tail -n +2 $RUN_PATH/nscf/"Q-points_IBZ.txt" | awk '{print $1, $2, $3}' >> $PREFIX.dyn0
    # Run q2r and matdyn
    echo "--> Running q2r.x and matdyn.x..."
    $Q2R_EXE < $RUN_PATH/$INPUT_DIR/q2r.in > q2r.out
    $MATDYN_EXE < $RUN_PATH/$INPUT_DIR/matdyn.in > matdyn.out
}
...
#    mkdir -p relax 
#    cd relax
#    $RUNNER $PW_EXE < $RUN_PATH/$INPUT_DIR/$PREFIX.relax.in > $PREFIX.relax.out
...
#    #awk '/Begin final coordinates/,/End final coordinates/ {if ($1 ~ /^[A-Z]/ && $1 != "CELL_PARAMETERS" && $1 != "ATOMIC_POSITIONS") print $0}' $PREFIX.relax.out > relaxed_coords.txt
#    awk '/Begin final coordinates/,/End final coordinates/ {if ($1 ~ /^[A-Z][a-z]?$/) print $0}' $PREFIX.relax.out > relaxed_coords.txt
...
#	    update_coords "$RUN_PATH/$INPUT_DIR/$PREFIX.relax.in" "$PREFIX.relax_relax.in"
...
#    update_coords "$RUN_PATH/$INPUT_DIR/$PREFIX.scf.in" "$RUN_PATH/scf/$PREFIX.scf_relaxed.in"
#    update_coords "$RUN_PATH/$INPUT_DIR/$PREFIX.nscf.in" "$RUN_PATH/nscf/$PREFIX.nscf_relaxed.in"
#    
#    cd $RUN_PATH
#    
...
#    $RUNNER $PW_EXE < $RUN_PATH/$INPUT_DIR/$PREFIX.nscf.in > $PREFIX.nscf.out
#fi
...
#$YAMBO_PH_EXE -i  &&
#$YAMBO_PH_EXE    &&
#$YPP_EXE -F $RUN_PATH/$INPUT_DIR/ypp_find_q.in > ypp_find_q.out
#
...
#echo "$NQ_IBZ" > $RUN_PATH/nscf/"Q-points_IBZ.txt"
#cat tmp_q.txt >> $RUN_PATH/nscf/"Q-points_IBZ.txt"
#rm tmp_q.txt
#cd $RUN_PATH
#
# ==============================================================================
...
cd ph_dvscf
#cp -r $RUN_PATH/scf/$OUTDIR/$PREFIX.save . 
#cp $RUN_PATH/$INPUT_DIR/$PREFIX.ph_dvscf.in $PREFIX.ph_dvscf_q.in
#cat $RUN_PATH/nscf/"Q-points_IBZ.txt" >> $PREFIX.ph_dvscf_q.in
#
